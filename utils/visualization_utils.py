...
def visualize_fence_on_image_array(image,
                                   boxes,
                                   classes,
                                   scores,
                                   category_index,
                                   class_names_to_detect,
                                   min_score_thresh,
                                   fence_mask,
                                   fence_color,
                                   force_draw):
  detected = False
  result_image = image.astype(np.uint32).copy()

  for i in range(boxes.shape[0]):
    ignore = True

    # Check class label is in detection list.
    class_name = category_index[classes[i]]['name']
    if not class_name in class_names_to_detect:
      continue
    ...
    print('Detected Object: {}, {}'.format(class_name, score))

    box = tuple(boxes[i].tolist())
    print(box)

    # TODO: Detect with a region of fence.
    detected = True
    break

  if force_draw or detected:
    # Draw electronic fence
    result_image = apply_mask(result_image, fence_mask, fence_color)
